#!/usr/bin/env python3
"""Test script to run caption processor on sample images."""
import sys
...
from uncloud.services.pipeline import ProcessingPipeline, MetadataService, PipelineStats
...
def main():
    ...
    db = SQLiteMediaRepository(db_path)
    
    # Setup metadata service
    console.print("[cyan]Initializing ExifTool daemon pool...[/cyan]")
    daemon_pool = ThreadLocalExifToolDaemon()
    metadata_service = MetadataService(daemon_pool)
    ...
    progress = RichProgressReporter(verbose=True)
    ...
    console.print(f"\n[bold cyan]Processing {len(image_files)} images...[/bold cyan]")
    ...
    with Progress(
        ...
        for i, image_path in enumerate(image_files, 1):
            try:
                ...
                if 'phash' in results or 'caption' in results:
                    record = MediaRecord(
    ...
    daemon = daemon_pool.get_daemon()
    subjects = daemon.extract_subjects(image_files[0])
    ...
    if total_records > 0:
        sample = db.get_by_path(str(image_files[0]))
        if sample:
